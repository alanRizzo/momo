...
const useAuthForm = () => {
	...
	return { formData, addressInput, setAddressInput, updateField, resetForm };
};

// ---------------------- VALIDACIÓN DE CAMPOS ----------------------
const validateField = (field: string, value: string | AddressType | null) => {
	if (!value) return "";
...
export function AuthModal({
	...
	const { formData, addressInput, setAddressInput, updateField, resetForm } =
		useAuthForm();
	...
	const {
		...
		clearSuggestions,
	} = useAddressSuggestions(addressInput, { countryCodes: "ar", limit: 5 });
	...
	const isFormValid = () => {
		...
		return requiredFields.every(
			(field) =>
				Boolean(formData[field as keyof typeof formData]) &&
				!validateField(field, formData[field as keyof typeof formData]),
		);
	};

	const handleBlur = (field: string, value: string | AddressType | null) => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			setFormErrors({
				general: err?.message || "Error inesperado. Intenta nuevamente.",
			});
		}
	};
										...
										{suggestions.map((s) => (
											<li
												...
												onClick={() => {
													...
													updateField("address", mapNominatimToAddress(s));
